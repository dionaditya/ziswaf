...
import {
  CreateDonationManualApiRequest,
  UpdateDonation
...
export const CorporateController = ({ children }) => {
  ...
  React.useEffect(() => {
    if (
      ...
      (async () => {
        ...
        const employee: any = await employeePresenter.loadDataDetail(
          _.toNumber(query["?employee_id"])
        );
  ...
  React.useEffect(() => {
    if (searchEmployeeDebounced !== "") {
      (async () => {
        const optionsEmployee = await employeePresenter.loadData({
          search: searchEmployeeDebounced,
        });
        if (
          _.isNil(optionsEmployee.data.data) === false &&
          optionsEmployee.data.data !== null
        ) {
  ...
  const postUpdateData = async () => {
    if (state.DonaturInfo.is_company) {
      const res = await corporatePresenter.storeNewData(
        new CreateDonorApiRequest(
          state.DonaturInfo.name,
          state.DonaturInfo.company_name,
          true,
